...
def earth_radius_at_lat(lat_microdeg: int) -> float:
    """
    Return Earth's radius (in µm) at a given latitude.
    Uses ellipsoid formula for oblate spheroid.
    """
...
def terrain_offset(lat_microdeg: int, lon_microdeg: int) -> int:
    ...
    Later: pull from DEM dataset (like SRTM/ETOPO).
    For now: returns 0.
    """
    return 0
...
def is_inside_earth(r_um: int, lat_microdeg: int, lon_microdeg: int) -> bool:
    ...
    optionally with local terrain adjustments.
    """
